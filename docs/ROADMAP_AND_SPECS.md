...
本書は `BLUEPRINT.md`・`RUNBOOK.md`・`CHECKLISTS.md` をはじめとする仕様・運用ドキュメントの交差点となる更新ハブです。各資料の改訂や新規タスク生成時に参照関係と整合性を即時確認できるよう、ロードマップと索引をまとめています。リリース準備・インシデント対応・設計着手など主要オペレーションの起点として読み返し、最新の導線を常に把握してください。
...
| 設計判断 | [docs/ADR/README.md](ADR/README.md) | 設計判断の記録・参照用途。 | 設計変更 PR で ADR を更新し、レビューへ添付する手順を徹底。 |
