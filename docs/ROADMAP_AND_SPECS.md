...
- [docs/birdseye/index.json](birdseye/index.json) / [birdseye/caps/](birdseye/caps/) / [tools/codemap/update.py](../tools/codemap/update.py)：Birdseye トポロジの鮮度維持に必要な生成物とスクリプトを束ねて確認。**利用シーン**：1. `generated_at` タイムスタンプを最新化。2. `CHECKLISTS.md` の[Hygiene](../CHECKLISTS.md#hygiene)で差分を照合。3. [tools/codemap/README.md#実行手順](../tools/codemap/README.md#実行手順)を参照し再生成。
...
- [EVALUATION.md#Test Outline](../EVALUATION.md#test-outline) / [tests/](../tests/)：テスト検証範囲と実装ディレクトリの対応を即座に把握。**利用シーン**：テスト追加前に評価指標・ケース網羅を確認し、TDD フローの起点として参照。
